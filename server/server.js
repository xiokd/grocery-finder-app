...
//get a stores by zip (Safeway or Yokes)
app.get("/groceryApp/store/item/:zip", async (req, res) => { //http://localhost:5555/
    try{
        const {zip} = req.params;
        const result = await pool.query("SELECT zip, city, address, store_name FROM store WHERE zip = $1", [zip]);
        console.log(result);
        console.log(zip);
        res.status(200).json({
            status: 200,
